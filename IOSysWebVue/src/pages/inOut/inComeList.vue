  ...
  <el-row class="warp">
    ...
    <el-col :span="24" class="warp-main">
      ...
      <el-table :data="list" :default-sort="{prop: 'amount', order: 'descending'}" show-summary :summary-method="doSum" highlight-current-row style="width: 100%;" :maxHeight="tableHeight" stripe>
        ...
        <el-table-column prop="inTypeName" label="类型" sortable></el-table-column>
      ...
      <!--对话框-->
      <InComeDetail ref="form" @saved="saved"></InComeDetail>
    </el-col>
  </el-row>
</template>
<script>
...
import InComeDetail from '@/pages/inOut/inComeDetail';
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      //查询收入列表
      InOutAPI.queryInCome(this.req.date).then(function(result) {
        if (result.isOK) {
          //列表设值
    ...
    deleteRecord: function(id) {
      ...
      this.$confirm("确认删除该记录吗?", "提示", { type: "warning" }).then(
        () => {
          //删除
          InOutAPI.deleteInCome({ id: id }).then(function(result) {
            if (result.isOK) {
              //成功提示
