  ...
  <el-row class="warp">
    ...
    <el-col :span="24" class="warp-main">
      ...
      >
        ...
        <el-table-column prop="fromAmountAccountName" label="源账户" sortable></el-table-column>
        <el-table-column prop="toAmountAccountName" label="目标账户" width="200" sortable></el-table-column>
        <el-table-column
          prop="amount"
          label="金额"
          width="200"
      ...
      <!--对话框-->
      <TransferDetail ref="form" @saved="saved"></TransferDetail>
    </el-col>
  </el-row>
</template>
<script>
...
import TransferDetail from "@/pages/inOut/transferDetail";
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      //查询列表
      InOutAPI.queryTransfer(this.req.date).then(function(result) {
        if (result.isOK) {
          //列表设值
    ...
    deleteRecord: function(id) {
      ...
      this.$confirm("确认删除该记录吗?", "提示", { type: "warning" }).then(
        () => {
          //删除
          InOutAPI.deleteTransfer({ id: id }).then(function(result) {
            if (result.isOK) {
              //成功提示
